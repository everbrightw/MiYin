...
public class TestUtils {
    ...
    private void askForStroagePermissions(){

    }

    //public static void uploadFile(String decodableString) {
    //    IMiVibeApi api = ApiClient.getVideoTestClient().create(IMiVibeApi.class);
    //    MediaType MEDIA_TYPE = MediaType.parse("video/mp4");
    //    File file = new File();
    //    RequestBody requestBody = RequestBody.create(MEDIA_TYPE, file);
    //    Call<ResponseBody> call = api.uploadVideo(TOKEN, requestBody);
    //    call.enqueue(new Callback<ResponseBody>() {
    //        @Override
    //        public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
    //            if(response.isSuccessful()){
    //                Log.i("YW_TEST", "ok" + response.code() + " "+ response.body().toString());
    //            }
    //            else{
    //                Log.i("YW_TEST", response.errorBody().toString());
    //            }
    //        }

    //        @Override
    //        public void onFailure(Call<ResponseBody> call, Throwable t) {
    //            Log.i("YW_TEST", "failed");
    //        }
    //    });
